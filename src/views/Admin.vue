        ...
        <b-card header-bg-variant="accent" class="border-0 shadow-sm" header-class="p-2">
            <template v-slot:header>
                <div class="mb-0 d-flex flex-row w-100 align-items-center">
                    <h5 class="mb-0 text-white">Admin Dashboard</h5>
...
export default {
    ...
    methods: {
        ...
        async getFiles() {
            const files = await this.$Amplify.Storage.list('');
            console.log(files);
            this.user.files = files.map(file => {
                console.log(file);
                file.owner = file.key.split('/')[0];
        ...
        onRowSelect(item) {
            ...
            this.loadDynamoFile(selectedRow.owner, selectedRow.key);
            this.showModal('File Description');
        },
        async loadDynamoFile(owner,key) {
            const apiName = 'vaultAPI';
            const apiPath = `/s3-files/object/${owner}~${key}`;
